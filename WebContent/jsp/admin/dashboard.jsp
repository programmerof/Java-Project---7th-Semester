...
<div class="main">
    <div class="page-header">
        ...
        <p>Manage your exams and monitor student performance.</p>
    </div>
    ...
    <%
        ...
        int total  = exams != null ? exams.size() : 0;
        long active = exams != null ? exams.stream().filter(Exam::isActive).count() : 0;
    %>
    ...
    <div class="stats-row">
        ...
        <div class="stat-card">
            <div class="stat-number"><%= total - active %></div>
            <div class="stat-label">Draft Exams</div>
        </div>
        <div class="stat-card" style="cursor:pointer" onclick="location.href='${pageContext.request.contextPath}/admin/results'">
            <div class="stat-number">&#128202;</div>
            <div class="stat-label">View All Results</div>
        </div>
    </div>

    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1rem;">
        <h2 style="font-family:'Syne',sans-serif; font-size:1.1rem;">All Exams</h2>
        <a href="${pageContext.request.contextPath}/admin/create-exam" class="btn btn-primary">+ Create New Exam</a>
    </div>

    <div class="card">
        <div class="table-wrap">
        <% if (exams == null || exams.isEmpty()) { %>
            <p style="color:var(--muted); padding:1rem 0;">No exams yet. <a href="${pageContext.request.contextPath}/admin/create-exam">Create your first exam</a>.</p>
        <% } else { %>
        <table>
            <thead>
                <tr><th>Title</th><th>Duration</th><th>Questions</th><th>Status</th><th>Actions</th></tr>
            </thead>
            <tbody>
            <% for (Exam exam : exams) { %>
                <tr>
                    <td><strong><%= exam.getTitle() %></strong>
                        <br><span style="color:var(--muted);font-size:0.78rem;"><%= exam.getDescription() != null ? exam.getDescription() : "" %></span>
                    </td>
                    <td><%= exam.getDurationMins() %> min</td>
                    <td><%= exam.getQuestionCount() %> Q</td>
                    <td><span class="badge <%= exam.isActive() ? "badge-active" : "badge-inactive" %>">
                        <%= exam.isActive() ? "Active" : "Draft" %>
                    </span></td>
                    <td style="display:flex; gap:0.4rem; flex-wrap:wrap;">
                        <a href="${pageContext.request.contextPath}/admin/questions?examId=<%= exam.getId() %>" class="btn btn-sm btn-success">Questions</a>
                        <form method="post" action="${pageContext.request.contextPath}/admin/dashboard" style="display:inline">
                            ...
                            <button class="btn btn-sm btn-warning" type="submit"><%= exam.isActive() ? "Deactivate" : "Activate" %></button>
                        </form>
                        <form method="post" action="${pageContext.request.contextPath}/admin/dashboard" style="display:inline"
                              onsubmit="return confirm('Delete this exam and all its questions/results?')">
                            <input type="hidden" name="examId" value="<%= exam.getId() %>">
