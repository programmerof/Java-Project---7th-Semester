...
<%@ page import="java.util.List, model.Exam, model.Result, model.User, dao.ResultDAO" %>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="main">
    ...
    <div class="page-header">
        ...
        <p>Pick an exam below to get started.</p>
    </div>

    <% if ("true".equals(request.getParameter("alreadyAttempted"))) { %>
        <div class="alert alert-info">&#9432; You have already attempted that exam.</div>
    <% } %>

    <h2 style="font-family:'Syne',sans-serif; font-size:1.1rem; margin-bottom:1rem;">Available Exams</h2>
    <% if (exams == null || exams.isEmpty()) { %>
        <div class="card" style="text-align:center; padding:2.5rem; color:var(--muted);">No exams available right now. Check back later!</div>
    <% } else { %>
    <div class="exam-grid">
    <% for (Exam exam : exams) {
           boolean attempted = results != null && results.stream().anyMatch(r2 -> r2.getExamId() == exam.getId());
    %>
        <div class="exam-card">
            <div class="exam-card-title"><%= exam.getTitle() %></div>
            <div style="color:var(--muted); font-size:0.85rem;"><%= exam.getDescription() != null ? exam.getDescription() : "" %></div>
            <div class="exam-card-meta">
                <span>&#128337; <%= exam.getDurationMins() %> min</span>
                <span>&#10067; <%= exam.getQuestionCount() %> questions</span>
            </div>
            <div class="exam-card-actions">
            <% if (attempted) { %>
                <span class="badge badge-active">&#10003; Attempted</span>
                <a href="${pageContext.request.contextPath}/student/results" class="btn btn-sm btn-success">View Result</a>
            <% } else { %>
                <a href="${pageContext.request.contextPath}/student/take-exam?examId=<%= exam.getId() %>" class="btn btn-primary btn-sm">Start Exam &#8594;</a>
            <% } %>
            </div>
        </div>
    <% } %>
    ...
    <h2 style="font-family:'Syne',sans-serif; font-size:1.1rem; margin:2rem 0 1rem;">Recent Results</h2>
    <% if (results == null || results.isEmpty()) { %>
        <div class="card" style="color:var(--muted); text-align:center; padding:1.5rem;">You haven't taken any exams yet.</div>
    <% } else { %>
    <div class="card">
        <div class="table-wrap">
        <table>
            <thead><tr><th>Exam</th><th>Score</th><th>Grade</th><th>Date</th><th></th></tr></thead>
            <tbody>
            <% for (Result r : results) { %>
                <tr>
                    <td><%= r.getExamTitle() %></td>
                    <td><%= r.getScore() %>/<%= r.getTotal() %> (<%= r.getPercentage() %>%)</td>
                    <td><span class="badge badge-<%= r.getGrade().toLowerCase().charAt(0) %>"><%= r.getGrade() %></span></td>
                    <td style="color:var(--muted);font-size:0.82rem;"><%= r.getSubmittedAt() %></td>
                    <td><a href="${pageContext.request.contextPath}/student/results?resultId=<%= r.getId() %>" class="btn btn-sm btn-success">Review</a></td>
                </tr>
            <% } %>
            </tbody>
        </table>
