...
<%@ page import="java.util.List, model.Result, model.Question" %>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Result Review — ExamPortal</title>
...
<div class="main">
    <%
        ...
        if (result == null) { response.sendRedirect(request.getContextPath() + "/student/results"); return; }
    %>
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1.5rem;">
        <div class="page-header" style="margin:0">
            <h1><%= result.getExamTitle() %></h1>
            <p>Submitted: <%= result.getSubmittedAt() %></p>
        </div>
        <a href="${pageContext.request.contextPath}/student/results" class="btn btn-danger btn-sm">&#8592; Back</a>
    </div>

    <div class="stats-row" style="margin-bottom:1.5rem;">
        <div class="stat-card">
            <div class="stat-number"><%= result.getScore() %>/<%= result.getTotal() %></div>
            <div class="stat-label">Score</div>
        </div>
        <div class="stat-card">
            <div class="stat-number"><%= result.getPercentage() %>%</div>
            <div class="stat-label">Percentage</div>
        </div>
        <div class="stat-card">
            <div class="stat-number"><%= result.getGrade() %></div>
    ...
    <%
        ...
        if (questions != null) {
            ...
            for (Question q : questions) {
                String status = q.getChosenOpt() == null ? "skipped" : (q.isCorrect() ? "correct" : "wrong");
    %>
        <div class="q-block <%= status %>">
            <div class="q-text">Q<%= num++ %>. <%= q.getQuestionText() %>
                <span style="float:right; font-size:0.82rem;">
                    <% if ("correct".equals(status)) { %><span style="color:var(--success)">&#10003; Correct</span>
                    <% } else if ("wrong".equals(status)) { %><span style="color:var(--error)">&#10007; Wrong</span>
                    <% } else { %><span style="color:var(--warning)">&#8212; Skipped</span><% } %>
                </span>
            </div>
            <div class="q-opts">
                <% String[] opts = {q.getOptA(), q.getOptB(), q.getOptC(), q.getOptD()};
                   String[] keys = {"A", "B", "C", "D"};
                   for (int i = 0; i < 4; i++) {
                       boolean isCorrect = keys[i].equals(q.getCorrectOpt());
                       boolean isChosen  = keys[i].equals(q.getChosenOpt());
                       String cls = isCorrect ? "opt-correct" : (isChosen && !isCorrect ? "opt-wrong" : "");
                %>
                <div class="opt-row <%= cls %>"><%= keys[i] %>. <%= opts[i] %>
                    <% if (isCorrect) { %> &#10003;<% } %>
                    <% if (isChosen && !isCorrect) { %> &#10007; (your answer)<% } %>
                </div>
                <% } %>
            </div>
        </div>
